...
def get_batting_data(soup):
    ...
    stats_tables = soup.findAll('table', {'class': 'stats_table'})
    # print(len(stats_tables))
    # this line goes wrong
    # he need the table 2 and 3
...
def get_game_summary(soup):
    ...
    # print(str(soup))
    # input("https://www.baseball-reference.com/b")

    strong_tag = scorebox.findAll('strong')

    href_he_need = []
    for strong in strong_tag:
        tag_a = strong.find('a')
        if(tag_a != None):
            href_he_need.append(tag_a['href'])

    # the 'href' he need: "/teams/SFG/2022.shtml"
    game['away_team_abbr'] = href_he_need[0].split('/')[2]

    # the href he need: "/teams/STL/2022.shtml"
    game['home_team_abbr'] = href_he_need[1].split('/')[2]
