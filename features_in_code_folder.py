...
def lambda_func_1(x):
    if(x.dtype == 'object'):
        x = x.str.replace("%", '').astype(float) / 100.0
        return x.rolling(200, min_periods=1).mean()
    return x.astype(float).rolling(200, min_periods=1).mean()


def lambda_func_2(x):
    if(x.dtype == 'object'):
        x = x.str.replace("%", '').astype(float) / 100.0
        return x.rolling(200, min_periods=1).std()
    return x.astype(float).rolling(200, min_periods=1).std()


def lambda_func_3(x):
    if(x.dtype == 'object'):
        x = x.str.replace("%", '').astype(float) / 100.0
        return x.rolling(200, min_periods=1).skew()
    return x.astype(float).rolling(200, min_periods=1).skew()
...
def add_season_rolling(stat_df, df, cols, team, name):
    ...
    for s in cols:
        if team:
            stat_df[s+'_mean'] = stat_df.groupby(['team', 'season'])[
                s].apply(lambda_func_1)
            stat_df[s+'_stdev'] = stat_df.groupby(['team', 'season'])[
                s].apply(lambda_func_2)
            stat_df[s+'_skew'] = stat_df.groupby(['team', 'season'])[
                s].apply(lambda_func_3)
        else:
            stat_df[s+'_mean'] = stat_df.groupby(['name', 'season'])[
                s].apply(lambda_func_1)
            stat_df[s+'_stdev'] = stat_df.groupby(['name', 'season'])[
                s].apply(lambda_func_2)
            stat_df[s+'_skew'] = stat_df.groupby(['name', 'season'])[
                s].apply(lambda_func_3)
...
def get_games():
    games = pd.read_csv(game_summaries_path)
    games.drop_duplicates()
    games.to_csv(game_summaries_path, index=False)
